    ...
    CREATE TABLE GASTADO_CLIENTE AS(
SELECT c.NOMBRE, SUM(lf.CANT*lf.PRECIO) AS TOTAL_GASTADO
...
GROUP BY c.NOMBRE
    ...
    ALTER TABLE PROVINCIAS ADD NUMERO_PUEBLOS NUMBER(10);

UPDATE PROVINCIAS P1 SET NUMERO_PUEBLOS = (
SELECT COUNT(p.CODPUE) AS NUMERO_PUEBLOS FROM PUEBLOS p, PROVINCIAS p2 WHERE p.CODPRO = p2.CODPRO AND P2.CODPRO=P1.CODPRO 
GROUP BY p2.CODPRO);
