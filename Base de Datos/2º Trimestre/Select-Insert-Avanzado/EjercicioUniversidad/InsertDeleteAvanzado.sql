    ...
    ALTER TABLE PROFESOR MODIFY ASIGNATURA VARCHAR2(30);

    UPDATE PROFESOR P2 SET ASIGNATURA = (SELECT A.NOMBRE FROM ASIGNATURA a WHERE A.IDPROFESOR =P2.IDPROFESOR  );
    ...
    UPDATE ASIGNATURA A2 SET TITULACION = (SELECT T.NOMBRE FROM ASIGNATURA a , TITULACION t 
    WHERE A.IDTITULACION = T.IDTITULACION AND A2.IDASIGNATURA =A.IDASIGNATURA) ;
    ...
    INSERT INTO PAGOS(NOMBRE_ALUMNO, PAGO) SELECT p2.NOMBRE ,  SUM(a3.COSTEBASICO*(((aa.NUMEROMATRICULA-1)*0.1)+1)) AS PAGO FROM ALUMNO a2 , ALUMNO_ASIGNATURA aa , PERSONA p2 , ASIGNATURA a3 
    WHERE p2.DNI = a2.DNI AND a2.IDALUMNO =aa.IDALUMNO AND aa.IDASIGNATURA =a3.IDASIGNATURA AND aa.NUMEROMATRICULA>=1  GROUP BY p2.NOMBRE, p2.DNI
